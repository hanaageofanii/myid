...
{
    protected static ?string $model = form_kpr_pca::class;

    protected static ?string $title = "Data Penjualan PCA";

    protected static ?string $navigationGroup = "KPR";

    protected static ?string $pluralLabel = "Data Penjualan KPR PCA";

    protected static ?string $navigationLabel = "Form Penjualan PCA";
    protected static ?string $navigationIcon = 'heroicon-o-banknotes';
    protected static ?string $pluralModelLabel = 'Daftar Data Penjualan KPR PCA';
            ...
            ->schema([
                Forms\Components\Select::make('jenis_unit')
                    ...
                    ->afterStateUpdated(function ($state, callable $set) {
                        if ($state) {
                            $bookedBlok = Pca::where('kpr_status', 'akad')
                                ->where('kavling', $state)
                                ->get('siteplan')
                                ->pluck('siteplan')
                                ->toArray();
                   ...
                   Forms\Components\Select::make('siteplan')
                    ...
                    ->options(function (callable $get) {
                        $jenisUnit = $get('jenis_unit'); // Ambil jenis unit yg dipilih
                
                        if (!$jenisUnit) {
                            return [];
                        }
                
                        return \App\Models\Pca::where('kpr_status', 'akad')
                    ...
                    ->afterStateUpdated(function($state, callable $set){
                        $pca = \App\Models\Pca::where('siteplan', $state)->first();
                
                        if ($pca) {
                            $set('tanggal_akad', $pca->tanggal_akad);
                            $set('tanggal_booking', $pca->tanggal_booking);
                            $set('nama_konsumen', $pca->nama_konsumen);
                            $set('agent', $pca->agent);
                            $set('luas', $pca->luas_tanah);
                            $set('type', $pca->type);
                            $set('status_akad', $pca->kpr_status);
                            $set('pembayaran', $pca->status_pembayaran);
                        }
                    })
